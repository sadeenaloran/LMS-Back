  ...
  try {
    ...
    if (!token) {
      return res.status(401).json(
        createResponse(false, "Authentication token missing", null, "No token provided")
      );
    }

    if (!process.env.JWT_SECRET) {
      return res.status(500).json(
        createResponse(false, "JWT secret not configured", null, "Server configuration error")
      );
    }
    ...
    if (!user) {
      return res.status(401).json(
        createResponse(false, "User not found", null, "Invalid token")
      );
    }
  ...
  return (req, res, next) => {
    if (!req.user || (roles.length && !roles.includes(req.user.role))) {
      return res.status(403).json(
        createResponse(false, "Unauthorized access", null, "Insufficient permissions")
      );
    }
    next();
  };
